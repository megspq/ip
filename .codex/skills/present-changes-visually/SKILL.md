...
4. Run the bundled generator from the repository root:
   ...
   Replace `HEAD`, `WORKTREE`, and the output path with the requested values. Comparison points can be any Git commit-ish, such as `HEAD~1`, a tag, a branch, or a commit SHA. Use `WORKTREE` for the current files.
...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator. The generated page remains usable offline, but its optional syntax highlighting loads highlight.js from a CDN.
